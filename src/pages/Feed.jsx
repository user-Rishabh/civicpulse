  ...
  const cycleStatus = (id) => {
    const updated = issues.map((issue) => {
      if (issue.id === id) {
        ...
        if (issue.status === "Pending") nextStatus = "In Progress";
        else if (issue.status === "In Progress") nextStatus = "Resolved";
        else if (issue.status === "Resolved") nextStatus = "Pending";
  ...
  return (
    <div className="pt-28 px-8 max-w-6xl mx-auto pb-20">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="bg-blue-500/20 text-blue-400 rounded-full px-3 py-1 text-sm font-semibold">
